...
For a candidate that changes architecture, verify the target's required proposal/decision evidence and its exact selected revision against the actual candidate and trusted base when that target supports such a protocol. Check that the PR identifies the applicable finalized plan or equivalent target-owned identity and that the target's native gate evaluated the intended transition. Keep the target protocol out of this generic method; missing, stale, or unsupported architecture evidence remains an explicit readiness gap.
