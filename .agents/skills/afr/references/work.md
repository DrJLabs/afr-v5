...
Use when dispatched by the AFR coordinator for authorized direct work or correction. Return the local candidate and evidence; activation, authority, phase sequencing, stop, resume, and completion belong to `SKILL.md`.
...
Choose a feature branch or isolated worktree when target policy, overlap, concurrency, or risk warrants it; reuse valid existing isolation. Do not create a worktree ritual for every change. Use native Git, verify the selected base and target instructions in the chosen checkout, and keep required local inputs available without copying unrelated edits. A new checkout from HEAD does not include uncommitted requirements or prerequisites. If those inputs cannot be safely preserved, expose the gap before editing.

For a non-Git target, use the host's available source/backup observations without inventing Git prerequisites. Do not imply recoverability that has not been established.
...
For each material requirement, identify what observation actually demonstrates it at the relevant consumer or integration boundary. Include preserved behavior and meaningful failure cases. Execute the narrowest reliable checks required by the target and selected assurance; inspect the commands, outputs, and assertions rather than treating a test name or exit code as complete coverage.

Distinguish **passed**, **failed**, **timed out**, **unavailable**, and **inconclusive** evidence. A check that did not run or lost its output is not a pass. Avoid installations, network access, full-suite reruns, or baseline rebuilds unless they are relevant and authorized. A required check that cannot be satisfied leaves a visible acceptance gap.
...
Report changed files and behavior, source/companion references, candidate/base identity, requirement coverage, actual commands/outcomes, coverage gaps, and residual concerns for review. Keep proposed checks distinct from executed checks. No mandatory evidence schema or separate plan file is required.

Reobserve tracked and untracked effects before returning. Account for caches, generated files, and temporary workspaces; prefer non-writing probes where practical. Remove only known task-created temporary artifacts when cleanup is authorized and safe. Preserve the deliverable candidate, unrelated changes, and any uncertain artifacts/worktrees; report their disposition rather than deleting them to make status clean. Return to the coordinator for review or a specific blocker, not an unsupported delivery action.
