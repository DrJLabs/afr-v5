...
**Status:** current design; R3 local work/review, R4 PR delivery, and R5 authorized umbrella continuation are implemented. Historical host and behavioral qualification remains bounded in the [R2 historical record](../tests/r2/README.md), [R3 trial record](../tests/r3/README.md), bounded [R4 trial record](../tests/r4/README.md), and separate [R5 trial record](../tests/r5/README.md). M8 source and the consumer gate/bootstrap are delivered and provider-qualified; exercised candidate architecture behaviors and one bounded combined acceptance are recorded for the completed umbrella. Broader release and qualification remain outside this evidence.
**Date:** 2026-09-15
...
### Architecture-aware adoption (M8 implemented; bounded qualification recorded)

The phase owners now carry a generic architecture-change contract through the existing workflow. Planning discovers the target-owned model, decisions, mappings, native rules, and checks; classifies ordinary versus material boundary changes; and requires a viewable proposal, complexity rationale, and explicit owner selection of the exact revision before dependent implementation. Work carries that selected identity, uses the target's own protocol and checks, and surfaces stale or changed boundaries. Review checks intended design against observed structure and behavior, cumulative complexity, source coverage, exceptions, and control edits. Delivery ties applicable architecture evidence and finalized target-owned identity to the actual candidate and trusted base. The coordinator reobserves that identity on umbrella continuation and resumption. These source changes are implemented in the five existing AFR owners and the Architecture Guard owner.

This guidance is deliberately protocol-independent. The consuming project owns proposal format, finalization, native commands, and any trusted gate. The guided native candidate trials covered greenfield planning and selection, ordinary conforming work, material transition, rejection/repair, and stale-plan revision handling within the recorded synthetic scope while preserving unchanged behavior and work; detailed evidence is in the [R5 trial record](../tests/r5/README.md). One scoped fault was observed when `uv` implicitly installed seven locked packages in a disposable fixture despite the no-install instruction; the parent agent caught the scope fault, the worker removed the fixture environment and local caches, and the worker reran through the existing interpreter. This records an intervention rather than flawless authority compliance. The governing prospective run under AFR revision `aa87adb` demonstrated a planned idle checkpoint and explicit operator resume with state reconciliation and no duplicate writes. The completed umbrella is counted once under that fixed governing revision, separately from candidate skill qualification; bounded combined acceptance was observed for this umbrella. Qualification remains limited to synthetic decisions and explicit path loading on this Codex surface; ChatGPT parity, global activation, and native interruption/crash recovery remain unqualified. The guidance does not add a second workflow, approval system, automatic rebaseline, or architecture runtime. A green model/parser/import check remains insufficient for behavior, and planning or rendering alone does not authorize implementation or establish approval.
...
The original v3 increment reconciled the design and evaluation boundary. R2 implementation and bounded observations remain historical evidence in the [R2 trial record](../tests/r2/README.md) for its recorded hashes. R3 and R4 package capabilities are described here and their observations belong in the [R3 trial record](../tests/r3/README.md) and [R4 trial record](../tests/r4/README.md); R5 capability is described here and its observations belong in the separate [R5 trial record](../tests/r5/README.md). Detailed M8 observations and limits remain in that R5 record. The completed umbrella provides bounded combined acceptance evidence for the fixed governing revision; do not infer broad release, native interruption/crash recovery, host qualification beyond the observed surface, or behavioral results from package presence. Installation/discovery claims, broader qualification, and performance claims remain limited by recorded evidence.
