...
**Status:** R3 local work/review, R4 PR delivery, and R5 authorized umbrella continuation implemented; R6 assessment complete with no helper extraction; M8 controlled architecture-change and AFR-adoption source implemented in the candidate package; consumer gate/bootstrap delivered and provider-qualified; exercised candidate architecture behaviors bounded-qualified, controlled material trial ongoing, with source delivery and combined acceptance pending
**Date:** 2026-09-15
**Repository stage:** coordinator, planning, work, review, and delivery references implemented; R5 umbrella continuation is implemented in the coordinator; M8 source spans the existing owners, and its bounded qualification is in progress

The [architecture v3 specification](architecture-v3.md) owns the current design and implementation boundary; the [skill package](../.agents/skills/afr/SKILL.md) owns the implemented coordinator and phase methods. The [donor matrix](donor-matrix.md) owns R1 evidence, extraction decisions, and evaluation scenarios. This roadmap owns milestones, sequencing, and complexity budgets. The 2026-09-15 reconciliation preserves the v1/v2 design and original R1 facts, carries the v3 planning/conformance improvements into R3, and records the explicit R2/R3/R4/R5, local/remote, and direct/umbrella boundaries without changing the package or complexity budgets. M8 is now an implemented source refinement of those owners. Its target-specific protocol and bounded qualification remain separately observed; the concise M8 boundary and evidence link are maintained in architecture-v3.
...
| `references/delivery.md` | Authorized PR creation/reuse, monitoring, observed exact-head merge, and synchronization |
...
The current workflow is defined by the [coordinator](../.agents/skills/afr/SKILL.md), [planning](../.agents/skills/afr/references/planning.md), [work](../.agents/skills/afr/references/work.md), [review](../.agents/skills/afr/references/review.md), and [delivery](../.agents/skills/afr/references/delivery.md) references. The coordinator owns authority, sequencing, umbrella continuation, stop/resume, combined acceptance, and reporting; references own phase methods. Explicit planning, research, and review-only requests do not authorize implementation or correction, while an authorized direct route may reach a verified local candidate and, when delivery is authorized and supported, a delivered PR through implementation, requirement-based verification, actual-diff inspection, proportional review, bounded correction, and exact-head delivery. Authorized umbrella work reuses that direct flow for dependency-eligible outcomes and retains exact revision/workspace boundaries. Planning artifacts remain permitted under the coordinator's stated authority conditions. Architecture-aware work additionally carries a target-owned viewable proposal, owner-selected revision, complexity rationale, and applicable native evidence through those same phases; target-specific finalization and gate protocols remain outside generic AFR. The architecture specification's [coordinator behavior](architecture-v3.md#4-coordinator-behavior) and [phase requirements](architecture-v3.md#5-phase-requirements) describe the current R5 behavior plus the implemented M8 source; the coordinator remains the operational owner. R2–R5 are historical/qualified package boundaries; M8 exercised behaviors are bounded-qualified while the controlled material trial remains ongoing. Combined acceptance and native interruption/crash recovery remain unqualified. The historical loop in §2 explains the donor rationale; it is not a separate operational instruction set.
...
The architecture's [resume requirements](architecture-v3.md#resume-and-evidence-reuse) use the plan, Git, PR observations, and host session. R1 and R5 select no dedicated state file or database; R5 selectively refreshes only work, dependencies, checks, review, or acceptance affected by changed inputs. Any later addition must satisfy §10 and the repository's explicit complexity test.
...
**Outcome (historical boundary):** one public `afr` skill can activate, inspect, route, and produce or assess an implementation-ready contract, a bounded research result, or a blocker without custom runtime state. R2 stops at the planning boundary.

**Implementation:** the original [coordinator](../.agents/skills/afr/SKILL.md), [planning reference](../.agents/skills/afr/references/planning.md), and explicit-only invocation metadata. The [R2 trial record](../tests/r2/README.md) owns observations for the package hashes recorded there. Those observations remain historical evidence and do not qualify the changed R3 package; current R3 evidence belongs in the [R3 trial record](../tests/r3/README.md). Presence of package files does not establish UI discovery, cross-host parity, or full runtime qualification.
...
**Outcome:** AFR completes one ordinary repository change from request through a verified local candidate or branch, without remote delivery or umbrella continuation.

This increment is the direct local work and review path; its implementation endpoint is a verified local candidate, not a remote delivery.
...
- one proportional independent review and one bounded correction when selected; and
- an honest completion or blocker result.

Explicit planning, research, and review-only requests do not authorize implementation or correction, or ordinary target-source edits. Planning artifacts remain allowed when explicitly requested, required by the target, or justified for resumability or risk within authority. An authorized direct implementation may apply bounded correction within that same implementation authority; a review-only request needs explicit fix authority before correction. The direct path may mutate only within the selected repository/workspace authority, preserve unrelated dirty work, and stop when authority, requirements, verification, or review cannot be satisfied. Run at least one real local change through the assembled planning/work/review path and compare acceptance, intervention, and process cost with an ordinary high-quality host-agent prompt on a comparable task/baseline. Include the case where selected tests pass but acceptance remains unmet. Record observations in the [R3 trial record](../tests/r3/README.md); do not treat the capability's presence as trial evidence or host qualification. Do not add PR automation, a database, helper, runtime, dependency, or SDK runner merely to complete this phase.

### R4 — PR convergence and delivery

**Outcome:** AFR can create or reuse and carry one authorized PR to a terminal delivery boundary, with a separately verified local synchronization result when applicable.

Deliver:

- concise PR creation context;
- exact-head observation;
- CI and review monitoring through native facilities or an external capability such as `babysit-pr`;
...
Exercise exact-head changes, missing/pending/failed/unknown checks, review feedback, and ambiguous external effects in an authorized disposable or controlled setting. Keep watcher-specific mechanics outside the canonical AFR skill. Any narrow helper must satisfy the architecture's extraction threshold and R6; a trial does not grant live delivery authority. Record bounded observations in the [R4 trial record](../tests/r4/README.md); its presence does not establish broad forge or host qualification.

### R5 — umbrella continuation

**Outcome:** implemented capability: AFR completes an authorized small multi-outcome plan without routine user babysitting between ordinary stages, while stopping at dependency, scope, authority, or safety blockers.
...
- exact dependency revision and workspace boundaries for dependent work;
...
- selective resume and correction when changed inputs affect only part of the umbrella;
- combined acceptance of the parent objective; and
- clean stop behavior for dependency, scope, authority, or safety blockers.

The coordinator owns this behavior and reuses the four unchanged phase references; no helper, runtime, scheduler, or state store is introduced. Exercise continuation and combined acceptance, including independently successful outcomes whose combined behavior fails, in the separate [R5 trial record](../tests/r5/README.md), which owns the bounded evidence and limits.
...
| Target package | 1 public skill and 4 phase references; similar per-reference line budget |
...
The R3 coordinator, planning reference, work reference, and review reference implement the direct local work/review boundary, R4 adds the delivery reference for one authorized PR, and R5 adds coordinator-owned umbrella continuation while reusing those four references. Keep discovery, host-support, and behavioral claims within the recorded trial evidence; [R2 results](../tests/r2/README.md) are historical for their recorded hashes, [R3 evidence](../tests/r3/README.md) covers local work/review, [R4 evidence](../tests/r4/README.md) covers bounded delivery trials, and [R5 evidence](../tests/r5/README.md) owns bounded continuation observations and limits. The [R6 assessment](r6-helper-assessment.md) selects no helper. The current increment is bounded qualification of the implemented M8 source; exercised candidate architecture behaviors are qualified for the recorded scope, while the controlled material trial remains ongoing. Its evidence, including one caught installation intervention and the explicit qualification limits, is recorded in the R5 trial record. Source delivery and parent combined acceptance remain pending. Installation/discovery remains separately authorized and qualified. R7 remains conditional on its representative-run threshold; this assessment does not authorize an SDK experiment.
