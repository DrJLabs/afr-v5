...
**Status:** R3 local work/review, R4 PR delivery, and R5 authorized umbrella continuation implemented; R6 assessment complete with no helper extraction; M8 architecture-aware adoption guidance drafted in the candidate package; host and behavioral qualification remains bounded
**Date:** 2026-09-15
**Repository stage:** coordinator, planning, work, review, and delivery references implemented; R5 umbrella continuation is implemented in the coordinator; M8 guidance spans the existing owners pending representative qualification

The [architecture v3 specification](architecture-v3.md) owns the current design and implementation boundary; the [skill package](../.agents/skills/afr/SKILL.md) owns the implemented coordinator and phase methods. The [donor matrix](donor-matrix.md) owns R1 evidence, extraction decisions, and evaluation scenarios. This roadmap owns milestones, sequencing, and complexity budgets. The 2026-09-15 reconciliation preserves the v1/v2 design and original R1 facts, carries the v3 planning/conformance improvements into R3, and records the explicit R2/R3/R4/R5, local/remote, and direct/umbrella boundaries without changing the package or complexity budgets. M8 guidance is an additive refinement of those owners; its target-specific protocol and end-to-end qualification remain separate.
...
The current workflow is defined by the [coordinator](../.agents/skills/afr/SKILL.md), [planning](../.agents/skills/afr/references/planning.md), [work](../.agents/skills/afr/references/work.md), [review](../.agents/skills/afr/references/review.md), and [delivery](../.agents/skills/afr/references/delivery.md) references. The coordinator owns authority, sequencing, umbrella continuation, stop/resume, combined acceptance, and reporting; references own phase methods. Explicit planning, research, and review-only requests do not authorize implementation or correction, while an authorized direct route may reach a verified local candidate and, when delivery is authorized and supported, a delivered PR through implementation, requirement-based verification, actual-diff inspection, proportional review, bounded correction, and exact-head delivery. Authorized umbrella work reuses that direct flow for dependency-eligible outcomes and retains exact revision/workspace boundaries. Planning artifacts remain permitted under the coordinator's stated authority conditions. Architecture-aware work additionally carries a target-owned viewable proposal, owner-selected revision, complexity rationale, and applicable native evidence through those same phases; target-specific finalization and gate protocols remain outside generic AFR. The architecture specification's [coordinator behavior](architecture-v3.md#4-coordinator-behavior) and [phase requirements](architecture-v3.md#5-phase-requirements) describe the current R5 behavior plus this M8 draft; the coordinator remains the operational owner. R2–R5 assemble the qualified package, while M8 adoption guidance awaits its representative run. The historical loop in §2 explains the donor rationale; it is not a separate operational instruction set.
