...
R1 inspected the mapped sources in `DrJLabs/afr` as immutable Git objects. The [architecture specification](architecture.md) is the resulting v5 design. The [roadmap](roadmap.md) controls subsequent delivery. This file is historical evidence and evaluation design; it is not loaded as a runtime workflow.
...
For each candidate, a later extraction decision must identify repeated v5 failures or measured waste, why the native alternative failed, the smaller helper interface, what existing repeated work it replaces, and how error rate or time/context savings will be measured. Regression tests in the donor alone do not meet that threshold.
...
These are specifications for later focused walkthroughs or tests and, once the core exists, integrated trials. Expected evidence is observable; matching a sentence in a skill is not a behavioral pass. No scenario below is marked passed by R1.
...
| E02 Honest routing | Compare a cohesive fix, several independent outcomes, missing architecture evidence, and a prohibited action | Record `direct`, `umbrella`, `spike`, or `stop` with a concrete reason; research ends with evidence and uncertainty; no speculative implementation |
...
| E05 Economical implementation | Use a large existing plan for one ordinary feature and a small prose change | Reuse sufficient planning; hand off paths only when useful; default to one implementer and proportional review; evidence covers the actual candidate |
| E06 Review follows risk | Compare prose, an ordinary feature, and an auth/persistence change; make a required reviewer unavailable | Select the lowest adequate review; preserve required checks; unavailable required review is visible, never claimed complete |
...
| E09 Resume without duplication | Interrupt after implementation, after review, and after PR creation; change the candidate or base in one case | Resolve current artifacts first; reuse applicable evidence; refresh invalidated checks/review; avoid duplicate implementation, branches, or PRs |
...
| E14 Verification coverage | A check fails, times out, loses output, or an unrelated successful check is offered for acceptance | Record what ran against which candidate; distinguish failure and unknown evidence from pass; require relevant acceptance coverage and explicit gaps |
| E15 Finish the requested scope | Review fixes are pushed or the first umbrella outcome merges with more eligible work remaining; contrast with a completed local-only request | Continue monitoring or the next authorized outcome; stop at the true requested endpoint; final report accurately identifies remaining work and delivery facts |
...
R1 produces two documents, updates the README and roadmap, and proposes one public skill with four references. Actual executable skill count remains zero, custom helper count zero, core services/databases zero, and custom runtimes zero. There is no runtime or verification-time measurement for v5 yet.

Verification for this increment covers exact donor revisions and file existence, Markdown links/anchors, matrix/scenario coverage, documentation consistency, and whitespace. Donor test assertions were inspected, not run. Neither live PR operations nor skill installation, end-to-end repository trials, interruption trials, or effectiveness benchmarks occurred. The next implementation boundary is R2.
